...
export default function GenerateStructuredPage() {
  ...
  async function onGenerate() {
    ...
    setResult(null);
    try {
      const resp = await fetch("/api/generate-structured", {
        method: "POST",
  ...
  return (
    <div className="mx-auto max-w-6xl p-6">
      <div className="mb-4 flex items-center justify-between">
        <div className="text-xl font-semibold">新方案：脚本理解 → 结构化布局 → 生图背景与元素 → 进入画布</div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-5">
        ...
        <div className="md:col-span-3">
          <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-4">
            ...
            {!result ? (
              <div className="mt-3 text-sm text-zinc-500">提交脚本后会在这里展示技术方案与预览图。</div>
            ) : (
